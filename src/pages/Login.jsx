    ...
    try {
      ...
      if (error) {
        ...
        if (error.message.includes('provider is not enabled') || error.message.includes('Unsupported provider')) {
          throw new Error('Google Sign-In is not configured yet. Please use Email/Password to sign in, or contact the administrator to enable Google authentication.')
        }
        throw error
      }
    } catch (error) {
        ...
        <div className="bg-white rounded-lg shadow-xl p-8">
          <div className="text-center mb-8">
            ...
            <p className="mt-2 text-gray-600">
              {isLogin
                ? 'Sign in to book your yoga classes'
          ...
          <div className="mt-6">
            ...
            <button
              ...
              disabled={loading}
              className="mt-4 w-full flex items-center justify-center gap-3 py-3 px-4 border border-gray-300 rounded-md shadow-sm bg-white text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 disabled:bg-gray-100 disabled:cursor-not-allowed transition-colors"
            >
